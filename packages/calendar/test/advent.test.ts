...
import { getChurchYear, getFirstSundayOfAdvent } from "@/calendar/temporal/mod.ts";
...
Deno.test("Advent Sundays in generated calendar", () => {
  const calendar = generateLiturgicalCalendar(2025, { includeCommemorationsAndLesserFestivals: false });
  const byName = new Map(calendar.map((event) => [event.name, event.date.toString()]));

  assertEquals(byName.get("First Sunday of Advent"), "2025-11-30");
  assertEquals(byName.get("Second Sunday of Advent"), "2025-12-07");
  assertEquals(byName.get("Third Sunday of Advent"), "2025-12-14");
  assertEquals(byName.get("Fourth Sunday of Advent"), "2025-12-21");
});
