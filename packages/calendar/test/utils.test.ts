import { validateDate, validateYear } from "@/calendar/temporal/mod.ts";
import type { DateInput } from "@/calendar/types.ts";
...
Deno.test("validateDate function", async (t) => {
  ...
  await t.step("PlainDateLike object input", async (t) => {
    ...
    await t.step("throws for incomplete PlainDateLike objects", () => {
      // Runtime guardrails; these shapes are not assignable to DateInput (PlainDateLike requires year, month, day).
      assertThrows(() => validateDate({ year: 2023, month: 5 } as DateInput), TypeError, "Invalid date");
      assertThrows(() => validateDate({ month: 5, day: 15 } as DateInput), TypeError, "Invalid date");
    });
  });
